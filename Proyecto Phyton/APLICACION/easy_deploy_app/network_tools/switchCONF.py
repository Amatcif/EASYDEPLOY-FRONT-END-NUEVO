...
def detectar_tipo_switch(con):
    ...
    if "alliedware" in texto or "allied telesis" in texto or "awplus" in texto:
        SWITCH_VENDOR = "allied"
        print("Switch Allied Telesis / AlliedWare Plus detectado.")
        return True

    prompt = enviar(con, "", espera_prompt=("#", ">"), timeout=3, mostrar=False).lower()
    if "awplus" in prompt:
        SWITCH_VENDOR = "allied"
        print("Switch Allied Telesis / AlliedWare Plus detectado por prompt.")
        return True

    if "cisco" in texto or "ios software" in texto or "cisco ios" in texto:
        print("Este equipo parece Cisco IOS. Vuelve a Redes y usa el boton 'Switch Cisco'.")
    else:
        print("No se ha detectado un switch Allied Telesis / AlliedWare Plus.")
...
def crear_vlan(con):
    ...
    salida = _crear_vlan_allied(con, vlan_id, nombre)
...
def configurar_puertos(con):
    puertos_str = pedir_texto("Puertos (ej: port1.0.1-1.0.4,port1.0.6): ", "puertos", 300, r"[A-Za-z0-9/._:,\- ]+")
    try:
    ...
    for interfaz in interfaces:
        ...
        if descripcion:
            enviar(con, f"description {descripcion}")
        if modo == "access":
            enviar(con, "switchport mode access")
            enviar(con, f"switchport access vlan {vlan_id}")
        elif modo == "trunk":
            enviar(con, "switchport mode trunk")
            enviar(con, f"switchport trunk allowed vlan add {vlans}")
        enviar(con, "no shutdown")
        enviar(con, "end")
        print(f"{interfaz} configurado como {modo}")
...
def _configurar_puertos_cisco(con, interfaces, descripcion, modo, vlan_id, vlans):
    ...
    for interfaz in interfaces:
        salida += enviar(con, f"interface {interfaz}")
        if descripcion:
        ...
        if modo == "access":
            ...
            salida_trunk = enviar(con, f"switchport trunk allowed vlan {vlans}")
            salida += salida_trunk
            if salida_tiene_error(salida_trunk):
                salida += enviar(con, f"switchport trunk allowed vlan add {vlans}")
...
def habilitar_ping_ssh(con):
    ...
    troncal = pedir_interfaz("Puerto troncal (ej. port1.0.x): ")

    enviar(con, "conf t")
    enviar(con, f"interface vlan{vlan_id}", espera_prompt="#")
    enviar(con, f"ip address {ip}{mascara}", espera_prompt="#")
    enviar(con, "exit", espera_prompt="#")
    enviar(con, f"ip route 0.0.0.0/0 {gateway}", espera_prompt="#")
    enviar(con, "service ssh", espera_prompt="#")
    ...
    try:
        ...
        if login(con):
            if detectar_tipo_switch(con):
                ...
                print("No se abre el menu porque el equipo no coincide con Switch Allied.")
        else:
